...
pub const PerlinGenerator = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, seed: u32) !PerlinGenerator {
        if (!initialized) {
            var i: usize = 0;
    ...
    pub fn deinit(self: *PerlinGenerator) void {
        if (self.perlin != null) {
            self.allocator.free(self.perlin.?);
        }
    }
    ...
    pub fn seedNoise(self: *PerlinGenerator, seed: u32) !void {
        ...
        if (self.perlin == null) {
            self.perlin = try self.allocator.alloc(f64, PERLIN_SIZE + 1);
        }
